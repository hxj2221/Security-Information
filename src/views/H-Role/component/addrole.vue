<template>
  <div class="addrole">
    ...
    <div style="min-height:510px">
      ...
      <div class="roleFs">
        <div class="H-edit">
          <div class="content" v-for="item in powlist" :key="item.id" :label="item.id">
            <div class="title">
              <el-checkbox :label="item.id" v-model="fcheck" @change="fcheckchange">{{ item.title }}</el-checkbox>
            </div>
            <div class="power" v-for="v in item._child" :key="v.id" :label="v.id">
              <el-checkbox v-model="vcheck" @change="vcheckchange" :label="v.id" class="tit">{{ v.title }}</el-checkbox>
              <el-checkbox-group v-model="checkList" @change="checkchange">
                <el-checkbox v-for="(vv, index) in v._child" :key="index" :label="vv.id">{{ vv.title }}</el-checkbox>
              </el-checkbox-group>
            </div>
          </div>
        </div>
