  ...
  <div>
    <div class="staffalBg" v-if="staffvue">
      ...
      <div class="staffIptsech">
        ...
        <el-select v-model="staffbeldepart" placeholder="请选择" class="staffSel" clearable @clear="delValue">
          <el-option label="请选择" value=""></el-option>
          <el-option v-for="item in optionbeldepart" :key="item.id" :label="item.title" :value="item.id">
          </el-option>
        </el-select>
        <el-input v-model="search" class="staffNameipt" placeholder="请输入员工姓名" clearable @clear="delValue"></el-input>
        <el-button class="searchbtn" type="primary" icon="el-icon-search" @click="seachAll()"></el-button>
      </div>
      <!-- 表格部分 -->
      <div class="staffIptTab">
        <el-table max-height="662" :data="tables" :header-cell-style="{ background: '#C2C5F6',color:'#000' }"
          ...
          <el-table-column label="员工状态">
            <template slot-scope="scope">
              <el-switch v-model="scope.row.status" :active-value="1" :inactive-value="2" active-color="#13ce66"
                inactive-color="#ff4949" @change="changeSwitch($event, scope.row)"></el-switch>
            </template>
          </el-table-column>

          <el-table-column fixed="right" label="操作">
            <template slot-scope="scope">
              <el-button class="staffFotedit" size="small" type="text"
                @click="handleEdit(scope.$index, scope.row, scope.row.id)">编辑</el-button>
              <el-button class="staffFotdel" size="small" type="text" @click="handleDelete(scope.$index, tables)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div class="staffpag">
        <div class="block">
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
            :current-page="currentPage" :page-sizes="nums" :page-size="num"
